...
fn windowed_col_iter() {
	let s = ContainerRM::from_vec(U1, Dynamic::new(4), &[1., 2., 3., 4.]);
	...
	let s = ContainerRM::from_vec(U1, Dynamic::new(40), &[0; 40]);
	let slice = s.slice_rows(0);
...
fn windowed_row_iter() {
	let s = ContainerCM::from_vec(Dynamic::new(4), U1, &[1., 2., 3., 4.]);
